-- 修复 case_import_optimizer Agent 数据
-- 版本: v1.1
-- 日期: 2026/05/07
-- 说明: 修复原 add_ai_import_optimize_agent.sql 中使用 name 替代 agent_code 的问题

-- 1. 删除旧的可能错误插入的记录（如果存在）
DELETE FROM `ai_sub_agent_config_files` WHERE agent_id IN (
  SELECT id FROM ai_sub_agents WHERE agent_code = 'case_import_optimizer' AND is_system = 1
);

DELETE FROM `ai_sub_agents` WHERE agent_code = 'case_import_optimizer' AND is_system = 1;

-- 2. 重新插入 Agent 主记录（使用正确的字段）
...
-- 4. soul.md
INSERT INTO `ai_sub_agent_config_files` (`agent_id`, `file_type`, `content`, `sort_order`) VALUES
